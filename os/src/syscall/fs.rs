...
pub fn sys_fstat(_fd: usize, _st: *mut Stat) -> isize {
    ...
    if let Some(file) = &inner.fd_table[_fd] {
        ...
        copy_to_user(token, _st as *mut u8, stat.as_bytes(), core::mem::size_of::<Stat>());
        0
    } else {
        -1
    }
...
pub fn sys_linkat(_old_name: *const u8, _new_name: *const u8) -> isize {
    ...
    if _old_name.is_null() || _new_name.is_null() || _old_name == _new_name {
        return -1;
    }
    let old_name = translated_str(current_user_token(), _old_name);
    let new_name = translated_str(current_user_token(), _new_name);
    if old_name.is_empty() || new_name.is_empty() || old_name.eq(&new_name) {
        return -1;
    }
